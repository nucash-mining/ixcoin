...
UniValue getblock(const UniValue& params, bool fHelp)
{
    ...
    // Verbosity levels, matching later Bitcoin Core releases:
    //   0 / false : hex-encoded block
    //   1 / true  : block object with transaction ids
    //   2         : block object with fully decoded transactions
    // The boolean form is still accepted so existing callers keep working.
    int verbosity = 1;
    if (params.size() > 1) {
        if (params[1].isNum())
...
UniValue getblockchaininfo(const UniValue& params, bool fHelp)
{
    ...
    // size_on_disk: total bytes of the block and undo files. Present in later
    // Bitcoin Core releases; backported so explorers can report chain size.
