...
/* OpenSSL < 1.1.0 has no EVP_MD_CTX_new/free; provide equivalents so the same
   code builds against both the modern system OpenSSL and the older OpenSSL
   used by the depends-based cross builds. */
...
bool PaymentRequestPlus::getMerchant(X509_STORE* certStore, QString& merchant) const
{
    ...
    try
    {
        ...
        bool signatureOK = ctx && pubkey &&
            EVP_VerifyInit_ex(ctx, digestAlgorithm, NULL) == 1 &&
            EVP_VerifyUpdate(ctx, data_to_verify.data(), data_to_verify.size()) == 1 &&
            EVP_VerifyFinal(ctx, (const unsigned char*)paymentRequest.signature().data(), (unsigned int)paymentRequest.signature().size(), pubkey) == 1;
