...
void RPCConsole::clear(bool clearHistory)
{
    ...
    // Rich-text documents do not inherit the application style sheet, so the
    // console has to be coloured from the active theme explicitly.
    ui->messagesWidget->document()->setDefaultStyleSheet(
        QString(
                ...
                "b { color: %4; } "
                  ...
                  QString("%1pt").arg(consoleFontSize),
                  Theme::color(Theme::Muted).name(),
                  Theme::color(Theme::Accent).name(),
                  Theme::color(Theme::Negative).name())
        );
